...
# # 일단 좌우 나눠서 중복으로 올라가는 코import cv2
# import torch
# import time
#
# # YOLOv5 모델 로드
...
#             else:
#                 elapsed_time_left = time.time() - start_time_left  # 경과 시간 계산
#                 print("Person detected at the left center for {:.2f} seconds".format(elapsed_time_left))
#         else:
#             # 중앙에서 벗어났으므로 시간 초기화
#             start_time_left = 0
#
#         # 객체가 오른쪽 중앙과 겹칠 때
#         if abs(center_x_right - bbox_center_x) < (frame.shape[1] // 4) and abs(center_y_right - bbox_center_y) < (frame.shape[0] // 4):
#             if start_time_right == 0:
#                 start_time_right = time.time()  # 시작 시간 기록
#             else:
#                 elapsed_time_right = time.time() - start_time_right  # 경과 시간 계산
#                 print("Person detected at the right center for {:.2f} seconds".format(elapsed_time_right))
#         else:
#             # 중앙에서 벗어났으므로 시간 초기화
#             start_time_right = 0
#
