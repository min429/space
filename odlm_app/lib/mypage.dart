...
import 'package:odlm_app/profile.dart';
...
class _MypageWidgetState extends State<MypageWidget> {
    ...
    try {
      ...
      setState(() {
        barChartData = List.generate(12, (index) {
          double value = monthlyTimes.length > index
              ? monthlyTimes[index] / 60
              : 0; // 시간 단위로 변환
          Color barColor = (index + 1) > thisMonth
              ? Colors.blue[100]!
              : Colors.lightBlueAccent; // 작년과 올해 색상 구분
          return BarChartGroupData(
            x: index + 1,
            barRods: [
              BarChartRodData(
                  toY: value, color: barColor, borderRadius: BorderRadius.zero),
            ],
          );
        });
      });
  ...
  Future<void> _fetchUserProfile() async {
    final String url =
        'http://10.0.2.2:8080/mypage/$userId'; // userId는 따로 정의되어 있다고 가정
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // 서버에서 받은 JSON 데이터를 Profile 객체로 변환
        setState(() {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body: userProfile == null // userProfile이 null일 때 로딩 상태를 표시
                      ...
                      Padding(
                        ...
                        child: Container(
                          padding: const EdgeInsets.symmetric(
                              horizontal: 16, vertical: 8), // 텍스트 주위 패딩 추가
                          decoration: BoxDecoration(
                            color: Colors.transparent, // 박스 배경색
                            borderRadius:
                                BorderRadius.circular(12), // 둥근 모서리 설정
                            border: Border.all(
                              // 테두리 색상 및 두께 설정
                              color: Color(0x806B7A9A), // 원하는 모서리 색깔
                              width: 2, // 테두리 두께
                            ),
                          ),
                          child: Text(
                            '등급: ${userProfile!.grade}',
